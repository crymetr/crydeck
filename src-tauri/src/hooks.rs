...
fn write_artifacts(dir: &PathBuf, port: u16, token: &str) -> Result<(), String> {
    ...
    // PSReadLine predictions are a try/catch ladder: HistoryAndPlugin needs
    // pwsh 7, History works on any PSReadLine 2.1+, and Windows PowerShell 5
    // just skips both. The claude wrapper resolves the real binary first so the
    // function cannot recurse into itself.
    let init_ps1 = format!(
...
$env:COCKPIT_SETTINGS = '{settings}'
$__cc = Get-Command claude -CommandType Application, ExternalScript -ErrorAction SilentlyContinue | Select-Object -First 1
if ($__cc) {{
  $global:__cockpit_claude = $__cc.Source
  function global:claude {{ & $global:__cockpit_claude --settings $env:COCKPIT_SETTINGS @args }}
}}
Write-Host "cockpit session | tab $env:COCKPIT_TAB_ID | 'claude' wired to hooks" -ForegroundColor DarkCyan
"#,
