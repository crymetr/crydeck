...
### Fixed
- **Model / effort picker did nothing on the first tab.** The injection helper (and `typeToActive`) guarded with `if (!id)`, but the backend hands the first session of every launch the id `0` — so `/model`, `/effort`, `/diff`, seeded prompts and auto-continue were all silently dropped on that tab. The guards now test for `null`, and back-to-back submits (e.g. "Follow default" sending `/model` then `/effort`) are serialized per session so they can't interleave into one broken line. The Enter is also sent as a separate keystroke so it doesn't race Claude's slash-command palette.
- **`set_output_style` could destroy an existing settings file.** A malformed, BOM-prefixed, or non-object `settings.local.json` was silently replaced with `{outputStyle}`; it now aborts instead of clobbering, and tolerates a UTF-8 BOM.
- **Context menus leaked listeners / clipped.** The `keydown` handler survived a click-outside close (holding the detached menu in memory); menus now clean up both listeners on every close and scroll instead of clipping when taller than the window.
- **Remote Control always showed the folder name ("dev") in the Claude app.** Sessions launched with the folder basename as a fixed Remote Control name, which pins it forever. The name is no longer forced, so Claude auto-names the session and updates it to reflect your first prompt, matching the topic instead of the folder.
- **Tab title showed Claude's spinner.** Claude Code prefixes the terminal title with its working spinner (`✳✳✳ …`); the tab now strips that prefix and shows the clean name.
- **Context menus clipped at the window edge.** The `diff`/`style` buttons sit near the right edge, so their menus now clamp back inside the window instead of running off-screen.
